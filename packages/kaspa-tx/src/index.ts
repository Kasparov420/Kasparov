...
import { encodeEvent, type ProtocolEvent } from "@kasparov/protocol";
...
export interface UTXO {
  txId: string;
  outputIndex: number;
  amount: bigint; // sompi (1 KAS = 100,000,000 sompi)
...
export class KaspaWallet {
  ...
  constructor(config: WalletConfig, nodeUrl: string = "https://api.kaspa.org") {
   ...
   */
  static async encryptMnemonic(mnemonic: string, password: string): Promise<string> {
   ...
   */
  static async decryptMnemonic(encrypted: string, password: string): Promise<string> {
   ...
   */
  async splitUTXOs(count: number = 10, amountPerUtxo: bigint = 10000000n): Promise<TxResult> {
   ...
   */
  async publishEvent(event: ProtocolEvent): Promise<TxResult> {
    try {
      // Encode event
   ...
   */
  private selectUTXO(): UTXO | null {
    if (this.utxoPool.length === 0) return null;
    
    // Sort by amount and pick a medium one
    const sorted = [...this.utxoPool].sort((a, b) => 
   ...
   */
  private async buildTransaction(input: UTXO, payload: Uint8Array): Promise<any> {
   ...
   * Sign transaction
   */
  private signTransaction(tx: any): any {
   ...
   */
  private async broadcast(tx: any): Promise<string> {
  ...
  private embedPayload(payload: Uint8Array): string {
