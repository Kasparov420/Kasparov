...
function validateChat(message: string): void {
...
export function decodeEvent(bytes: Uint8Array): ProtocolEvent {
  ...
  switch (eventType) {
    case "G": {
      ...
      if (!tsMatch) throw new ValidationError("Invalid timestamp format");
      return {
        type: "game-init",
        gameId,
        whitePub,
    ...
    case "M": {
      ...
      if (!nMatch) throw new ValidationError("Invalid ply number format");
      return {
        type: "move",
        gameId,
        uci,
    ...
    case "C": {
      ...
      const nMatch = parts[4]?.match(/^n=(\d+)$/);
      if (!nMatch) throw new ValidationError("Invalid seq number format");
      return {
        type: "chat",
        gameId,
        message,
